<template>
  <div class="min-h-screen bg-slate-50 text-slate-900">
    <div class="mx-auto max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div class="flex items-start gap-3">
          <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50 ring-1 ring-emerald-100">
            <svg class="h-5 w-5 text-emerald-600" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <rect x="3" y="7" width="18" height="12" rx="2" stroke-width="2" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 14h4" />
            </svg>
          </div>
          <div>
            <h1 class="text-xl font-semibold tracking-tight">Cuentas Bancarias</h1>
            <p class="mt-1 text-sm text-slate-500">
              <span class="font-semibold text-emerald-700">{{ cuentasFiltradas.length }}</span> visibles ·
              <span class="font-semibold text-emerald-700">{{ cuentas.length }}</span> total
            </p>
          </div>
        </div>
      ...
      <div class="mt-5">
        <!-- Empty: no data -->
        <div v-if="cuentas.length === 0" class="rounded-2xl bg-white p-10 text-center shadow-sm ring-1 ring-slate-200">
          ...
          <h3 class="mt-4 text-base font-semibold">Sin cuentas bancarias</h3>
          <p class="mt-1 text-sm text-slate-500">Crea las cuentas a las que reciben transferencias (ej: Cuenta BBVA, Cuenta Azteca).</p>
          <button
            @click="abrirModal()"
        ...
        <!-- Empty: no results -->
        <div v-else-if="cuentasFiltradas.length === 0" class="rounded-2xl bg-white p-10 text-center shadow-sm ring-1 ring-slate-200">
            ...
            <table class="min-w-full border-separate border-spacing-0">
              <thead class="sticky top-0 z-10 bg-white">
                <tr>
                  ...
                  <th class="border-b border-slate-200 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Cuenta / CLABE</th>
                  <th class="border-b border-slate-200 px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Titular</th>
              ...
              <tbody>
                <tr
                  ...
                  :key="c.id"
                  @dblclick="abrirModal(c)"
                  class="group border-b border-slate-200 hover:bg-slate-50"
                  :class="{ 'opacity-70': !c.activo }"
                >
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-2">
                      <span class="font-semibold text-slate-900">{{ c.nombre }}</span>
                      <span v-if="!c.activo" class="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">
                        Inactiva
                      </span>
                    </div>
                  </td>
                  ...
                  <td class="px-4 py-3 text-slate-600">
                    <div class="flex flex-col text-xs">
                      <span v-if="c.numero_cuenta">Cuenta: {{ c.numero_cuenta }}</span>
                      <span v-if="c.clabe">CLABE: {{ c.clabe }}</span>
                      <span v-if="!c.numero_cuenta && !c.clabe">—</span>
                    </div>
                  </td>

                  <td class="px-4 py-3 text-slate-600">{{ c.titular || '—' }}</td>

                  <td class="px-4 py-3 text-center">
                    <span
                      class="inline-flex rounded-full px-3 py-1 text-xs font-semibold ring-1"
                      :class="c.activo ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-slate-100 text-slate-600 ring-slate-200'"
                    >
            ...
            <div class="w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-slate-200">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    <h2 class="text-base font-semibold tracking-tight">
                      {{ modal.editando ? 'Editar cuenta' : 'Nueva cuenta bancaria' }}
                    </h2>
                    <p class="mt-1 text-sm text-slate-500">Define los datos de la cuenta que recibe transferencias.</p>
                  </div>
                </div>
              ...
              <div class="space-y-4 px-5 py-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  ...
                  <!-- Titular -->
                  <div>
                    <label class="text-sm font-semibold text-slate-700">Titular</label>
                    <input
                      v-model.trim="form.titular"
                      maxlength="150"
                      class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100"
                  ...
                  <!-- Número de cuenta -->
                  <div>
                    <label class="text-sm font-semibold text-slate-700">Número de cuenta</label>
                    <input
                      v-model.trim="form.numero_cuenta"
                      maxlength="30"
                      class="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none transition focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100"
                ...
                <div class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4">
                  <div>
                    ...
                    <div class="mt-1 text-sm text-slate-500">{{ form.activo ? 'La cuenta estará activa.' : 'La cuenta quedará inactiva.' }}</div>
                  </div>
              ...
              <div class="flex items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 px-5 py-4">
                ...
                >
                  ...
                  {{ modal.editando ? 'Guardar cambios' : 'Crear cuenta' }}
                </button>
              </div>
            </div>
          </div>
...
const form    = reactive({ nombre: '', banco: '', numero_cuenta: '', clabe: '', titular: '', activo: true })
const errores = reactive({ nombre: '' })

const API = '/api/cuentas-bancarias'
...
async function cargarCuentas() {
  try {
    ...
    Toast.fire({ icon: 'error', title: 'Error al cargar cuentas bancarias' })
  }
}
cargarCuentas()
...
function abrirModal(c = null) {
  ...
  form.banco          = c?.banco ?? ''
  form.numero_cuenta   = c?.numero_cuenta ?? ''
  form.clabe           = c?.clabe ?? ''
  form.titular         = c?.titular ?? ''
  form.activo          = (c && c.activo !== undefined && c.activo !== null) ? !!c.activo : true

  errores.nombre = ''
  modal.editando = !!c
  modal.idEditando = c?.id ?? null
  modal.mostrar = true
...
async function confirmarEliminar(c) {
  const r = await Swal.fire({
    ...
    html: `<p style="color:#475569;font-size:.9rem">Se eliminará <strong>${c.nombre}</strong>.</p>`,
    icon: 'warning',
