...
import { confirm, error, toastSuccess } from "@/lib/alert";
...
const paginacion=reactive({current_page:1,last_page:1,total:0});
...
async function cargar(page=1){if(!props.cliente?.id)return;loading.value=true;try{const{data}=await http.get(`/api/clientes/${props.cliente.id}/estado-cuenta`,{params:{...filtros,page}});resumen.value=data.resumen;movimientos.value=data.movimientos.data;paginacion.current_page=data.movimientos.current_page;paginacion.last_page=data.movimientos.last_page;paginacion.total=data.movimientos.total;pedidos.value=(data.saldos_pedido||[]).filter(p=>Number(p.saldo)>0);}catch{error("Error","No se pudo cargar el estado de cuenta.");}finally{loading.value=false;}}
async function ajustar(){
    const monto=Number(ajuste.monto||0);
    const verbo=ajuste.naturaleza==="credito"?"aumentar":"disminuir";
    const ok=await confirm({title:"¿Registrar ajuste de saldo?",text:`Vas a ${verbo} el saldo general de ${props.cliente?.nombre} en ${money(monto)}. Motivo: ${ajuste.motivo||"(sin motivo)"}`,confirmText:"Sí, registrar",tone:ajuste.naturaleza==="credito"?"primary":"warning",icon:"question"});
    if(!ok)return;
...
watch(()=>[props.open,props.cliente?.id],([open])=>{if(open)cargar(1);});
</script>
