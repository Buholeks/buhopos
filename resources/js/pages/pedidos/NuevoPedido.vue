<template>
    <main class="mx-auto max-w-6xl space-y-4">
        ...
        <form class="rounded-2xl border border-slate-200 bg-white shadow-sm" @submit.prevent="guardarEncargo">
            ...
            <div class="space-y-4 p-4">
                ...
                <div class="grid gap-3 lg:grid-cols-[minmax(0,1fr)_44px] lg:items-end">
                    ...
                    />
                    <button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-slate-600 transition hover:bg-slate-50" title="Crear producto rapido" @click="abrirProductoRapido">
                        <Plus class="h-4 w-4" />
                    </button>
                </div>
        ...
        <Teleport to="body">
            <div v-if="modalRapido.mostrar" class="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/40 p-4" @mousedown.self="cerrarProductoRapido">
                <div class="w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                    <div class="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
                        <div>
                            <h2 class="text-base font-black text-slate-900">Crear producto rapido</h2>
                            <p class="mt-0.5 text-xs text-slate-500">Crea producto padre, variante y lo agrega al pedido.</p>
                        </div>
                        <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" title="Cerrar" @click="cerrarProductoRapido">
                            <X class="h-4 w-4" />
                        </button>
                    </div>

                    <form class="space-y-4 p-5" @submit.prevent="crearProductoRapido">
                        <BaseInput v-model.trim="productoRapido.nombre" label="Producto" placeholder="Ej. Tenis Nike Air talla 26" required autofocus />

                        <div class="grid gap-3 sm:grid-cols-2">
                            <BaseInput v-model.trim="productoRapido.codigo" label="Codigo" placeholder="Opcional" />
                            <BaseInput v-model.number="productoRapido.precio_venta" label="Precio venta" type="number" min="0" step="0.01" required />
                        </div>

                        <div class="grid gap-3 sm:grid-cols-1">
                            <BaseInput v-model.number="productoRapido.cantidad" label="Cantidad" type="number" min="1" step="1" required />
                        </div>

                        <div v-if="tiposAtributo.length" class="grid gap-3 sm:grid-cols-2">
                            <BaseSearchSelect
                        ...
                        <div v-else class="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                            No hay atributos activos configurados. Crea atributos como color o talla antes de usar este flujo.
                        </div>

                        <label class="block">
                            <span class="mb-1 block text-xs font-bold text-slate-600">Descripcion del pedido</span>
                            <textarea v-model="productoRapido.descripcion_pedido" rows="2" class="w-full resize-none rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100" placeholder="Opcional; solo se guarda en este pedido" />
                        </label>

                        <div class="flex justify-end gap-2 border-t border-slate-100 pt-4">
                            <button type="button" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" @click="cerrarProductoRapido">
                                Cancelar
                            </button>
                            <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-60" :disabled="guardandoRapido || !productoRapido.nombre || Number(productoRapido.cantidad) < 1 || !atributosCompletos">
                                <Loader2 v-if="guardandoRapido" class="h-4 w-4 animate-spin" />
                                Crear y agregar
                            </button>
                        </div>
                    </form>
        ...
        <Teleport to="body">
            <div v-if="modalVariante.mostrar" class="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/40 p-4" @mousedown.self="cerrarModalVariante">
                <div class="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                    <div class="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
                        <div>
                            <h2 class="text-base font-black text-slate-900">Seleccionar variante</h2>
                            <p class="mt-0.5 text-xs text-slate-500">{{ modalVariante.producto?.nombre }}</p>
                        </div>
                        <button type="button" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" title="Cerrar" @click="cerrarModalVariante">
                            <X class="h-4 w-4" />
                        </button>
                    </div>

                    <div class="max-h-[70vh] space-y-4 overflow-y-auto p-5">
                        <div v-if="cargandoVariantes" class="flex justify-center py-6">
                            <Loader2 class="h-5 w-5 animate-spin text-slate-400" />
                        </div>

                        <div v-else>
                            <div v-if="modalVariante.variantes.length" class="space-y-2">
                                <button
                                    ...
                                    @click="seleccionarVarianteExistente(variante)"
                                >
                                    <span class="font-bold text-slate-800">{{ variante.nombre_variante || variante.sku }}</span>
                                    <span class="text-xs font-bold text-emerald-700">Seleccionar</span>
                                </button>
                            </div>

                            <div v-else class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-6 text-center text-sm text-slate-500">
                                Este producto todavia no tiene variantes.
                            </div>

                            <div class="border-t border-slate-100 pt-4">
                                <h3 class="text-sm font-black text-slate-900">Nueva variante</h3>
                                <div v-if="tiposAtributo.length" class="mt-3 grid gap-3 sm:grid-cols-2">
                                    <BaseSearchSelect
                                        v-for="tipo in tiposAtributo"
                                        :key="tipo.id"
                                        v-model="modalVariante.atributos[tipo.id]"
                                        :label="tipo.nombre"
                                        placeholder="Seleccionar"
                                        :items="tipo.atributos ?? []"
                                        label-key="valor"
                                ...
                                <div v-else class="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                    ...
                    <div class="flex justify-end gap-2 border-t border-slate-100 bg-slate-50 px-5 py-4">
                        <button type="button" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50" @click="cerrarModalVariante">
                            Cancelar
                        </button>
                        <button type="button" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-black text-white hover:bg-emerald-700 disabled:opacity-60" :disabled="guardandoVariante || !atributosVarianteCompletos" @click="crearYSeleccionarVariante">
                            <Loader2 v-if="guardandoVariante" class="h-4 w-4 animate-spin" />
                            Crear y seleccionar
                        </button>
                    </div>
                </div>
...
const atributosCompletos = computed(() => {
    ...
    return tipos.length > 0 && tipos.every((tipo) => productoRapido.atributos[tipo.id])
})

const atributosVarianteCompletos = computed(() => {
    const tipos = tiposAtributo.value ?? []
    return tipos.length > 0 && tipos.every((tipo) => modalVariante.atributos[tipo.id])
})
...
async function abrirModalVariante(producto) {
    ...
    try {
        ...
        const { data } = await http.get(`/api/pedidos/productos/${producto.producto_id}/variantes`)
        ...
        toastError(e?.response?.data?.message || 'No se pudieron cargar las variantes')
        cerrarModalVariante()
    } finally {
...
async function crearYSeleccionarVariante() {
    if (!modalVariante.producto?.producto_id || !atributosVarianteCompletos.value) {
    ...
    try {
        const { data } = await http.post(`/api/pedidos/productos/${modalVariante.producto.producto_id}/variantes`, {
...
async function cargarAtributosRapidos() {
    ...
    try {
        ...
        tiposAtributo.value = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []
    } catch {
...
async function crearProductoRapido() {
    if (!productoRapido.nombre || Number(productoRapido.cantidad || 0) < 1 || !atributosCompletos.value) {
    ...
    try {
        ...
        toastError(errores?.codigo?.[0] || e?.response?.data?.message || 'No se pudo crear el producto')
    } finally {
