<template>
    <div class="min-h-screen bg-slate-50">
        ...
        <div class="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur">
            <div class="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2 px-3 sm:px-6 py-3 sm:py-4">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-cyan-100 text-cyan-700">
                        <ClipboardList class="h-5 w-5" />
                    </div>
                    <div>
                        <h1 class="text-lg font-semibold tracking-tight text-slate-900">Consulta de caja</h1>
                        <p class="text-xs text-slate-500">Movimientos y ventas registrados por fecha</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mx-auto max-w-7xl px-3 sm:px-6 py-4 sm:py-6 space-y-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7">
            ...
            <!-- RESUMEN -->
            <div v-if="resumen && !cargando" class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p class="text-xs font-medium text-slate-500">Total registros</p>
                    <p class="mt-1 text-xl font-bold text-slate-900">{{ paginacion.total ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-emerald-100 bg-emerald-50 p-4 shadow-sm">
                    <p class="text-xs font-medium text-emerald-600">Ingresos (página)</p>
                    <p class="mt-1 text-xl font-bold text-emerald-700">{{ fmt(resumen.ingresos) }}</p>
                </div>
                <div class="rounded-xl border border-rose-100 bg-rose-50 p-4 shadow-sm">
                    <p class="text-xs font-medium text-rose-600">Egresos (página)</p>
                    <p class="mt-1 text-xl font-bold text-rose-700">{{ fmt(resumen.egresos) }}</p>
                </div>
                <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                    <p class="text-xs font-medium text-slate-500">Saldo neto (página)</p>
                    <p
                        class="mt-1 text-xl font-bold"
                        :class="resumen.neto >= 0 ? 'text-emerald-700' : 'text-rose-700'"
                    >
                        {{ fmt(resumen.neto) }}
                    </p>
                </div>
            ...
            <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                >
                    ...
                    <p class="text-sm font-medium">Sin registros para los filtros seleccionados</p>
                </div>
                    ...
                    <table class="w-full text-sm">
                        <thead class="border-b border-slate-100 bg-slate-50 text-xs font-semibold uppercase tracking-wider text-slate-500">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left">Concepto / Folio</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr
                                ...
                                :class="mov.tipo === 'ingreso' ? 'bg-emerald-50/20' : 'bg-rose-50/20'"
                            >
                                ...
                                <td class="whitespace-nowrap px-4 py-3">
                                    <span
                                        ...
                                        :class="mov.origen === 'venta'
                                            ? 'bg-cyan-100 text-cyan-700'
                                            : 'bg-slate-100 text-slate-600'"
                                    >
                                        <ShoppingCart v-if="mov.origen === 'venta'" class="h-3 w-3" />
                                        <SlidersHorizontal v-else class="h-3 w-3" />
                                ...
                                <td class="whitespace-nowrap px-4 py-3">
                                    ...
                                    >
                                        <TrendingUp v-if="mov.tipo === 'ingreso'" class="h-3 w-3" />
                                ...
                                <td class="whitespace-nowrap px-4 py-3">
                                    <span
                                        ...
                                        :class="{
                                            'bg-slate-100 text-slate-700':   mov.forma_pago === 'efectivo',
                                            'bg-blue-50 text-blue-700':      mov.forma_pago === 'tarjeta',
                                            'bg-violet-50 text-violet-700':  mov.forma_pago === 'transferencia',
                                            'bg-amber-50 text-amber-700':    mov.forma_pago === 'credito',
                                        }"
                                    >
                                        <Banknote v-if="mov.forma_pago === 'efectivo'" class="h-3 w-3" />
                                        <CreditCard v-else-if="mov.forma_pago === 'tarjeta'" class="h-3 w-3" />
                                        <ArrowLeftRight v-else-if="mov.forma_pago === 'transferencia'" class="h-3 w-3" />
                                ...
                                <td
                                    ...
                                    :class="mov.tipo === 'ingreso' ? 'text-emerald-700' : 'text-rose-700'"
                                >
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <template v-for="p in paginasVisibles" :key="p">
                            ...
                            <button
                                ...
                                :class="p === paginacion.current_page
...
import {
    ...
    Banknote, CreditCard, ArrowLeftRight, Wallet,
...
const paginacion     = ref({ current_page: 1, last_page: 1, total: 0, from: 0, to: 0 })
...
async function cargarUsuarios() {
    try {
        const { data } = await http.get('/api/users', { params: { por_pagina: 200 } })
...
async function buscar(pagina = 1) {
    ...
    try {
        ...
        Swal.fire({
            icon:  'error',
            title: 'Error',
            text:  e.response?.data?.message ?? 'No se pudieron cargar los registros.',
...
function calcularResumen() {
    ...
    const egresos = movimientos.value
        ...
        .reduce((s, m) => s + Number(m.monto), 0)
    resumen.value = { ingresos, egresos, neto: ingresos - egresos }
}

function limpiar() {
    filtros.value = { desde: hoy, hasta: hoy, origen: '', tipo: '', forma_pago: '', concepto: '', user_id: '' }
    buscar()
}
...
function etiquetaFormaPago(f) {
    return { efectivo: 'Efectivo', tarjeta: 'Tarjeta', transferencia: 'Transferencia', credito: 'Crédito' }[f] ?? f
}
</script>
