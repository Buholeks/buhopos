    ...
    <div class="min-h-screen bg-slate-50 text-slate-900">
        ...
        >
            ...
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-500">
                            Consulta rápida por marca, modelo, talla, color y
                            existencia.
                        </p>
                    </div>
                </div>
                ...
                <div
                    class="hidden rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600 sm:block"
                >
        ...
        <main class="mx-auto max-w-7xl space-y-5  py-5 ">
            <section
            ...
            >
                ...
                >
                    ...
                    <label class="flex items-end">
                        <span
                            class="flex h-10 w-full items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm text-slate-700"
                        >
                            <input
                                v-model="f.con_stock"
                                type="checkbox"
                ...
                <div
                    v-if="filtros.atributos.length"
                    class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3 xl:grid-cols-5"
                >
                    <label
                        v-for="tipo in filtros.atributos"
                        :key="tipo.id"
                        class="block"
                    >
                        <span
            ...
            >
                ...
                >
                    ...
                    <div class="space-y-3 p-4">
                        ...
                        <div
                            class="flex items-end justify-between gap-3 border-t border-slate-100 pt-3"
                        >
                            <div>
                                <p
                            ...
                            <span
                                class="text-xs font-semibold text-slate-400 group-hover:text-emerald-600"
                            >
                                Ver precios
                            </span>
                        </div>
            ...
            >
                ...
                <p class="mt-1 text-sm text-slate-500">
                    Ajusta talla, color, marca o búsqueda para ampliar
                    resultados.
                </p>
            </section>
            ...
            >
                ...
                <div class="space-y-4 p-5">
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <Info
                            label="Modelo"
                            :value="seleccionado.modelo || '-'"
                        />
                        <Info
                            label="SKU"
                            :value="
                                seleccionado.sku || seleccionado.codigo || '-'
                            "
                        />
                        <Info
                            label="Existencia"
                            :value="fmtStock(seleccionado.stock)"
                        />
                    </div>
                    ...
                    >
                        <table class="min-w-full text-sm">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                <tr
                                    v-for="p in seleccionado.precios"
                                    :key="p.key"
                                >
                                    <td
...
const modelosFiltrados = computed(() => {
    ...
    return filtros.modelos.filter(
        (m) => Number(m.marca_id) === Number(f.marca_id),
    );
});

const chips = computed(() => {
    const out = [];
    const marca = filtros.marcas.find(
        (m) => Number(m.id) === Number(f.marca_id),
    );
    const modelo = filtros.modelos.find(
        (m) => Number(m.id) === Number(f.modelo_id),
    );
    ...
    Object.values(f.atributo_ids)
        ...
        .forEach((id) => {
            for (const tipo of filtros.atributos) {
                const attr = tipo.atributos?.find(
                    (a) => Number(a.id) === Number(id),
                );
                if (attr)
                    out.push({
                        key: `attr-${id}`,
                        label: `${tipo.nombre}: ${attr.valor}`,
                    });
            }
        });
